...
tech_levels = {
    # Ticker: [R1, R2, S1, S2] (R=ต้าน, S=รับ) -> แก้ไข index: S1=2, S2=3, R1=0, R2=1
    # Format ใน code: [ต้าน1, ต้าน2, รับ1, รับ2]
...
# --- ส่วนขวา: Watchlist (พร้อม Logic Sniper) ---
with col_side:
    ...
    for t in sorted(list(set(my_watchlist_tickers))): 
        ...
        if s1 > 0:
            ...
            if price <= s1:
                signal = "✅ IN ZONE" # เข้าโซนรับ
            elif 0 < dist_to_s1 <= 2.0: # ใกล้แนวรับไม่เกิน 2%
                signal = "🟢 ALERT" 
            elif price >= r1:
                signal = "🔴 PROFIT" # ชนต้าน
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "Signal": st.column_config.Column("Action", width="small"),
