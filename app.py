...
# CSS ปรับแต่งเล็กน้อยให้สวยขึ้น
st.markdown("""
<style>
...
def get_live_data(stock_data):
    ...
    for t in ticker_list:
        try:
            ...
            live_prices[t] = hist['Close'].iloc[-1] if not hist.empty else 0
        except:
            live_prices[t] = 0
...
# คำนวณยอดรวมเพื่อใช้ใน Scorecard ด้านบน
...
col_m3.metric("🇺🇸 Net Worth (USD)", f"${total_value_usd:,.2f}", f"${(total_value_usd - df['Cost USD'].sum()):,.2f}")
...
def display_styled_table(sub_df, title):
    ...
    display_df = sub_df[['Ticker', 'Company', 'Qty', 'Avg Cost', '%G/L', 'Total Gain USD', 'Value USD', '%Port', 'Change']].copy()
    display_df.columns = ['Ticker', 'Company', 'Qty', 'Avg Cost', '%G/L', 'Total Gain', 'Value', '%Port', 'Note']
